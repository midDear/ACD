...
module EffectUtils {
    ...
    export function blinkEffect(obj, interval: number = 1000): void {
        new displayobjectBlink(obj, interval);
    }

    //抖动对象特效
    //类似ios密码输入错误的特效
    export function shakeObj(obj, type = 1): void {
        var shakeNum = 80;
        var old: number = obj.x;
        if (type == 1) {
            ...
            egret.setTimeout(function () {
                egret.Tween.get(obj).to({ x: old }, shakeNum);
            }, this, shakeNum * 5);
        } else {
            old = obj.y;
            egret.Tween.get(obj).to({ y: obj.y - 10 }, shakeNum);

            egret.setTimeout(function () {
                egret.Tween.get(obj).to({ y: obj.y + 20 }, shakeNum);
            }, this, shakeNum * 2);
            egret.setTimeout(function () {
                egret.Tween.get(obj).to({ y: obj.y - 20 }, shakeNum);
            }, this, shakeNum * 3);
            ...
            egret.setTimeout(function () {
                egret.Tween.get(obj).to({ y: old }, shakeNum);
            }, this, shakeNum * 5);
        }
    }
    ...
    //抖动对象特效
    // 1：抖动  2：震动
    export function shakeScreen(obj: any, effectType: number = 1): void {
        var panel = obj;
        var shakeNum = 40;
    ...
    */
    export function playEffect(obj, cartoonType: number = 1): void {
        ...
        var onComplete1: Function = function () {
            ...
            }else if (cartoonType == 4) {
                egret.Tween.get(obj).to({ scaleX: 1, scaleY: 1, x: obj.x - obj.width* 0.1, y: obj.y- obj.height*0.4 }, 650, egret.Ease.bounceOut).call(onComplete2, this);
            }
        };
        if(cartoonType == 4) egret.Tween.get(obj).to({ scaleX: 0.8, scaleY: 0.6, x: obj.x+ obj.width *0.1, y: obj.y + obj.height*0.4 }, 100, egret.Ease.sineIn).call(onComplete1, this);
        else egret.Tween.get(obj).to({ scaleX: 0.5, scaleY: 0.5, x: obj.x + obj.width / 4, y: obj.y + obj.height / 4 }, 100, egret.Ease.sineIn).call(onComplete1, this);
    }
